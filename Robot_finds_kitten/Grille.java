 ...
 * */
public class Grille{
	public int nbrPiecesX;//nombre de piece horizontal
	public int nbrPiecesY;//nombre de piece vertical
	public int largeurPiece;//largeur de piece
	public int hauteurPiece;//hauteur de piece
	public Case[][] grille;//le map
	public Point terminal;//la position du kitten
	public String nomKitten;//nom du kitten
	...
	/*
	set le grille
	@param grille (le nouveau grille)*/
	...
	*@param nbrPiecesX nombre de piece horizontal
	*@param nbrPiecesY nombre de piece vertical
	*@param largeurPiece largeur de la piece
	*@param hauteurPiece hauteur de la piece
	*@param nbrNonKitten nombre de item
	*
	*/
	public Grille(int nbrPiecesX, int nbrPiecesY, int largeurPiece , int hauteurPiece, int nbrNonKitten){
		...
		grille = new Case [nbrPiecesY*(hauteurPiece-1)+1][nbrPiecesX * (largeurPiece-1)+1];
		...
		for (int i = 0; i < grille.length; i++){
			for (int j = 0; j < grille[0].length; j++){
				...
				if (i != 0 && i != grille.length-1 && i % (hauteurPiece-1) == 0 && j % (largeurPiece-1) == largeurPiece/2)
					grille[i][j] = new Porte();
				if (j != 0 && j != grille[0].length-1 && j % (largeurPiece-1) == 0 && i % (hauteurPiece-1) == hauteurPiece/2)
					grille[i][j] = new Porte();
			}
		}
		//cle
		//algorithms pour avoir une cle dans chaque piece
		for (int i = 0; i < nbrPiecesY; i++){
			for (int j = 0; j < nbrPiecesX; j++){
				int y = (int)(Math.random()*(hauteurPiece-2)+1)+i*(hauteurPiece-1);
				int x = (int)(Math.random()*(largeurPiece-2)+1)+j*(largeurPiece-1);
				grille[y][x] = new Cle();
			}
		}
		//kitten
		Kitten kitten = new Kitten ("le chaton");
		nomKitten = kitten.getNom();
		kitten.representation = "/images/nki/"+kitten.getRandomSymbole()+".png";
		int x=0, y=0;		
		while(grille[y][x].representation != "/images/back.png"){//boucle pour trouver une case vide
			x = (int)(Math.random()*(nbrPiecesX * (largeurPiece-1)+1));
			y = (int)(Math.random()*(nbrPiecesY * (hauteurPiece-1)+1));
		}
		grille[y][x] = kitten;
		terminal = new Point (x, y);
		
		//NonKitten
		int nbDescription = descriptions.length;
		NonKitten[] item = new NonKitten [nbrNonKitten];
		for (int i = 0; i < nbrNonKitten; i++){
			x=0;y=0;
			item[i] = new NonKitten (descriptions[i]);
			item[i].representation = "/images/nki/"+item[i].getRandomSymbole()+".png";
			while(grille[y][x].representation != "/images/back.png"){//boucle pour trouver une case vide
			    x = (int)(Math.random()*(nbrPiecesX * (largeurPiece-1)+1));
			    y = (int)(Math.random()*(nbrPiecesY * (hauteurPiece-1)+1));
		    }
		...
		//teleporteur
		Teleporteur teleport = new Teleporteur ();
		x=0; y=0;		
		while(grille[y][x].representation != "/images/back.png"){//boucle pour trouver une case vide
			x = (int)(Math.random()*(nbrPiecesX * (largeurPiece-1)+1));
			y = (int)(Math.random()*(nbrPiecesY * (hauteurPiece-1)+1));
	...
	/*
	retourne une case vide
	@return Point les coordonnees dont la case est vide*/
	public Point randomEmptyCell(){
		Point random = new Point (0, 0);
		int x = nbrPiecesX * (largeurPiece-1)+1;
		int y = nbrPiecesY * (hauteurPiece-1)+1;
		while (grille[random.getY()][random.getX()].representation != "/images/back.png"){
			random = new Point ((int)Math.floor(Math.random()*x), (int)Math.floor(Math.random()*y));
		}
		return random;
	}
	...
	public boolean deplacementPossible(Robot robot, int x, int y){
		if (grille[y][x].representation == "/images/wall.png" || (grille[y][x].representation == "/images/door.png" && robot.getNbClef() == 0))
			return false;
		return true;
	}
	...
	public void afficher( Robot robot){
		...
		for (int i = 0; i < grille.length; i++){
			for (int j = 0; j < grille[0].length; j++)
				if(robot.getPosition().egal(j, i))
					System.out.print("#");
				else
				    System.out.print(grille[i][j].representation);
			System.out.print("\n");
		}
	}
	
	/*
	return la position de certain objet
	@param symbole le symbole de l'objet
	@return Point(x,y) les coordonnee*/
	public Point coordonnee(String symbole){
		int x=-1, y=-1;
		for (int i = 0; i < grille.length; i++){
			for (int j = 0; j < grille[0].length; j++){
				if (grille[i][j].representation == symbole){
					x = j;
					y = i;
				}
			}
	...
	/*
	interaction entre le robot et les objets NonKitten et Kitten
	@param robot
	@print afficher la description de l'objet non-kitten et le message du succes
	
    theoriquement, la methode doit tenir en compte de l'interaction entre le robot et 
	les cles, les porte et la teleporteur. Or ceci est deja fait dans la classe Robot
	@see Robot#trouverCleOuPorte(Case[][])
	@see Robot#trouverTele(Case[][])
	*/
	public String interagir(Robot robot){
		if(grille[robot.getPosition().getY()][robot.getPosition().getX()] instanceof NonKitten){
			NonKitten temp = (NonKitten)grille[robot.getPosition().getY()][robot.getPosition().getX()];
	...
	/*
	tableau de description des objets non-kitten*/
