...
st_process_requests (server_thread * st, int socket_fd)
{
  ...
  for (;;) {
    ...
    printf("thread = %d : Requete reçu de:  %s\n",st->id ,buffer);
    if (len > 0) {
...
while(count_dispatched < nb_registered_clients);
...
st_open_socket (int port_number)
{
  ...
  listen (server_socket_fd, 10);//server_backlog_size);
}
                        ...
                        int nb_ressources,  int nb_process)
{
  ...
  for(int j = 0; j< nb_process; j++)
  {
     for(int i = 0; i< nb_process; i++)
     {   
        if(fini[i] == 0 && vector_inferieur_egale(besoins_client[i], disponible, nb_ressources))
        {
